...
while IFS= read -r file; do
  tex_files+=("$file")
done < <(find "$CURRENT_DIR" -maxdepth 1 -type f -name 'paper1_nrr-core_v*.tex' | LC_ALL=C sort)

pdf_files=()
while IFS= read -r file; do
  pdf_files+=("$file")
done < <(find "$CURRENT_DIR" -maxdepth 1 -type f -name 'paper1_nrr-core_v*.pdf' | LC_ALL=C sort)
